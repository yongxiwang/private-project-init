...
module.exports = function(project, version) {
    ...
    function startCreateFiles() {
        ...
        promiseArr.push.apply(promiseArr, [createGitigonreFile(), createPackageFile(), createGruntFile()]);
    ...
    function createGitigonreFile() {
        createFile('.gitignore', 'gitignore.tmpl');
    }

    function createPackageFile() {
        createFile('package.json', 'package.tmpl');
    }

    function createGruntFile() {
        createFile('Gruntfile.js', 'gruntfile.tmpl');
    }

    function createFile(src, tmpl) {
        info.log('+ ' + path.basename(src).cyan);
        return readFile(templatesDir + '/' + tmpl, 'utf8').then(function(data) {
            return writeFile(workDir + '/' + src, _.template(data, { project: project, version: version }), 'utf8');
        });
    }
